...
select type , count([customer city]) Count_of_orders, round(SUM(sales-[Order Item Discount]),2) as Total_Sales
from Supply_Chain
group by Type
...
select top(5) [Customer City], round(sum(Sales-[Order Item Discount]),2) Total
from Supply_Chain
...
WHERE [order City] IN (
    ...
    from Supply_Chain
    group by [Customer City]
    order by round(sum(Sales-[Order Item Discount]),2) desc
);
...
select [Customer Full Name], round(sum(Sales-[Order Item Discount]),0) Sales
from Supply_Chain
...
select [Customer Full Name], [Customer City], round(sum(Sales-[Order Item Discount]),0) Sales
from Supply_Chain
...
-- What is the lowes ordered products to make offers on them 
